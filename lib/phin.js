...
const phin = async (opts) => {
	...
	let url = typeof opts === 'object' ? opts.url : opts;

	if (opts.query) {
		const hasQueryString = url.includes('?')
		const extendURLSymbol = hasQueryString ? '&' : '?'
		const newQueryString = new URLSearchParams(opts.query).toString();
		url = `${url}${extendURLSymbol}${newQueryString}`;
	}

	const req = centra(url, opts.method || 'GET')
